...
int R_TreeWidget(const Node* node, float x, float y) {
	...
	int hovered = ctx->mouseX >= x - w && ctx->mouseX <= x + w &&
	...
	uint32_t color = (ctx->activeId == node->id) ? 0xFFFF0000 :
		(ctx->hotId == node->id) ? 0xFFFFFF00 : node->color;

	CTX_AddCmd(ctx, (DrawCmd) { CMD_CIRCLE, x, y, w, 0, color, NULL });
	CTX_AddCmd(ctx, (DrawCmd) { CMD_NUM, x, y, 0, 0, FONT_COLOR, node->val });
...
void R_AddConnectingLine(LayoutNode* layout, int childIdx) {
	...
	CTX_AddCmd(ctx, (DrawCmd) { CMD_LINE, prevPos.x, prevPos.y, child->x, child->y, FONT_COLOR, 3 });
}
