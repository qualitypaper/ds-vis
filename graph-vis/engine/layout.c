...
static F32 layout_core(Node* node, F32 depthY, F32* cursorX, UIContext* ctx, LayoutNode* out)
{
	out->node = node;
	out->y = depthY;
	out->children[0] = NULL;
	...
	for (size_t i = 0; i < node->childCount; i++) {
		...
		LayoutNode* child = PushStruct(ctx->frameArena, LayoutNode);
		out->children[i] = child;
		F32 cx = layout_core(node->children[i], depthY + LEVEL_SPACING, cursorX, ctx, child);
...
static void shift_x(LayoutNode* n, F32 dx)
{
	if (!n) return;
	n->x += dx;
...
F32 layout_tree(Node* node, F32 depthY, F32* cursorX, UIContext* ctx, LayoutNode* out)
{
	/* pack from 0, then translate so the root (not the leftmost child) lands on *cursorX */
	F32 desiredX = *cursorX;
	F32 packX = 0.0f;
	F32 rootX = layout_core(node, depthY, &packX, ctx, out);
