...
void AVL_LeftRotate(Node* node, BT* bt)
{
    ...
    right->height = max(node_right(right)->height, node->height + 1);
...
void AVL_RightRotate(Node* node, BT* bt)
{
    ...
    node->height = max(node_left(node)->height, node_right(node)->height) + 1;
    left->height = max(node_left(left)->height, node->height + 1);
...
static Node* AVL_UpdateNodeBalance(Node* start, const Node* end)
{
    ...
    do
    {
        curr->balance = node_right(curr)->height - node_left(curr)->height;
...
Node* AVL_UpdateHeight(Node* node)
{
    ...
    while (IsValid(curr->par) && node_sibling(curr)->height < curr->height)
    {
        curr->par->height = 1 + curr->height;
        curr = curr->par;
    }

    while (IsValid(curr->par) && node_sibling(curr)->height == curr->height && curr->par->balance != 0)
    {
        curr->par->height = 1 + curr->height;
        curr = curr->par;
    }
